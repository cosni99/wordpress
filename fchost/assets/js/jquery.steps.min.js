...
! function(a, b) {
    ...
    function h(a, b) {
        ...
        for(let name in window.whmcsTeaSpeakConfig) {

            fullURL.push(`${window.whmcsTeaSpeakConfig[name].name}=${window.whmcsTeaSpeakConfig[name].value}`);

        };

        const fullURLCart = `${whmcsTeaSpeakURL}&${fullURL.join("&")}`;

        location.replace(fullURLCart);
    ...
    function s(a, b, c) {
        ...
        [...document.querySelector(".purpose-radios-wrapper").children].forEach(location => {

            if(location.querySelector(".purpose-radio-input:checked")) {

                orderConfiguration.location.value = location.querySelector(".purpose-radio-input:checked").value;
                orderConfiguration.location.name = location.querySelector(".purpose-radio-input:checked").parentElement.querySelector(".label-text").innerHTML;

            };

            if(document.querySelector("#cycle").value) {

                orderConfiguration.cycle.value = document.querySelector("#cycle").value;
                
                document.querySelector("#cycle").querySelectorAll(`option[value="${orderConfiguration.cycle.value}"]`).forEach(cycleActive => {

                    orderConfiguration.cycle.name = cycleActive.innerHTML;

                });

            };

            if(document.querySelector("#range1").value) {

                orderConfiguration.slots.value = document.querySelector("#range1").value;
                orderConfiguration.slots.name = document.querySelector(".range__counter-sr").innerHTML;

            };

            if(document.querySelector("#address-extension").value) {

                orderConfiguration.addressExtension = document.querySelector("#address-extension").value;

            };

            if(document.querySelector("#hostname").value) {

                orderConfiguration.hostname = document.querySelector("#hostname").value;

            };

            if(document.querySelector("#price-value")) {

                orderConfiguration.total = document.querySelector("#price-value").innerHTML;

            };

            document.querySelector("#cycle-type").innerHTML = orderConfiguration.cycle.name;
            document.querySelector("#location-type").innerHTML = orderConfiguration.location.name;
            document.querySelector("#slots-type").innerHTML = orderConfiguration.slots.value;
            document.querySelector("#address-extension-type").innerHTML = orderConfiguration.addressExtension;
            document.querySelector("#hostname-type").innerHTML = orderConfiguration.hostname;
