<?php //functions.php 全てのテンプレートに確実に読み込まれる
...
function twpp_enqueue_styles() {
  ...
  wp_enqueue_style( 'animate', $uri.'/assets/css/animate.min.css' );
  wp_enqueue_style( 'icons', $uri.'/assets/icons/crowbarcode-icons.css' );
  wp_enqueue_style( 'hover', $uri.'/assets/css/hover-min.css' );
  wp_enqueue_script('jquery', $uri.'/assets/js/jquery-3.6.0.slim.min.js');
...
if (function_exists('register_sidebar')) {
  register_sidebar(array('id'=>'slider-1','name' => 'サイド1'));
  register_sidebar(array('id'=>'footer-1','name' => 'サイド2'));
  register_sidebar(array('id'=>'footer-2','name' => 'サイド3'));
...
class My_Widget extends WP_Widget{
    ...
    function update($new_instance, $old_instance) {
        if(empty($new_instance['team_img']) || empty($new_instance['team_name'])|| empty($new_instance['team_name'])){
            return false;
        }
        return $new_instance;
    }
